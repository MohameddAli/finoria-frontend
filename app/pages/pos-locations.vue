    ...
    <v-main>
      <v-container fluid class="pa-0">
        ...
        <v-row no-gutters class="content-row">
          ...
          >
            <div class="sidebar-search-action mb-3">
              <v-text-field
                v-model="search"
...
import { useLocationsData, type LocationPoint } from '~/composables/useLocationsData';

type ATM = LocationPoint;
...
const nearbyCount = computed(() => {
  ...
  return filteredAtms.value.filter(a => (a.distanceKm ?? Infinity) <= radiusKm.value).length;
});
...
function resetFilters() {
  ...
  nearMe.value = Boolean(userLocation.value);
  radiusKm.value = DEFAULT_RADIUS_KM;
...
.sidebar-search-field :deep(.v-field) {
  border-radius: 12px;
}
