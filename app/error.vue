...
onMounted(async () => {
  if (error.value) {
    ...
    if (error.value.statusCode === 401) {
      ...
      toast.error(t('errors.message.unauthorized'))
      clearError()
...
const errorIconColor = computed(() => {
  const colors = {
    ...
    forbidden: isDark.value ? '#ffb74d' : '#f57c00',
...
const handleRetry = async () => {
  ...
  try {
    ...
    toast.error(t('errors.action.retry'))
  } finally {
...
const copyErrorDetails = async () => {
  if (import.meta.client) {
    ...
    try {
      ...
      toast.error('Error details copied to clipboard')
    } catch (err) {
