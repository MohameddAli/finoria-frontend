...
export const useExport = () => {
  ...
  const validateExportData = (
    ...
    if (!columns || columns.length === 0) {
      toast.warning("No columns specified for export");
      return false;
    }
    ...
    if (!options.filename || options.filename.trim() === "") {
      toast.warning("Please provide a filename for the export");
      return false;
    }
  ...
  const exportDataClient = async (
    ...
    try {
      ...
      toast.error(`Export failed: ${exportError.value}`);
      throw error;
    }
  };
  ...
  const exportToExcelClient = async (
    ...
    const worksheetData = options.includeHeaders ? [headers, ...rows] : rows;
  ...
  const exportToPDFClient = async (
    ...
    if (typeof (doc as any).autoTable === "function") {
      ...
      throw new Error(
        "autoTable function not available. jspdf-autotable plugin may not be properly loaded."
      );
    }
  ...
  const cancelExport = async (jobId?: string) => {
    ...
    toast.warning("Client-side exports cannot be cancelled once started");
  };
