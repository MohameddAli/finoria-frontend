      ...
      <template v-for="(item, index) in navigationItems" :key="index">
        ...
        >
          <template #activator="{ props }">
            ...
            <v-list-item v-else v-bind="props" class="nav-item" rounded="lg">
              <template #prepend>
                <v-icon :icon="item.icon" size="20" />
              </template>
          ...
          <template v-for="child in getChildren(item)" :key="child.value">
            <v-list-item
              v-if="railMode"
              :to="child.to"
              :value="child.value"
              ...
              <div class="d-flex align-center">
                ...
                <div class="user-details flex-grow-1">
                  <div class="user-name text-body-2 font-weight-medium">
                    {{ authStore.admin?.name || "Guest" }}
                  </div>
                  <div class="user-email text-caption text-medium-emphasis">
                    {{ authStore.admin?.email || "guest@example.com" }}
                  </div>
                </div>
                <v-btn
                  icon
...
.nav-item:hover:not(.nav-item-active) {
  background-color: rgba(var(--v-theme-primary-hover), 0.1);
...
/* Ensure active title keeps on-primary color */
.nav-item-active .nav-title {
  color: rgb(var(--v-theme-on-primary));
}

/* Preserve active styles on hover explicitly */
.nav-item-active:hover {
  background-color: rgb(var(--v-theme-primary));
  color: rgb(var(--v-theme-on-primary));
}

.nav-item-active:hover :deep(.v-icon),
