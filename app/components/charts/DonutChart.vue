...
const props = defineProps({
  series: { type: Array, default: () => [44, 55, 13, 43, 22] },
  labels: { type: Array, default: () => ['A', 'B', 'C', 'D', 'E'] },
  title: { type: String, default: '' },
  height: { type: [Number, String], default: 320 },
  width: { type: [Number, String], default: '100%' },
  elevation: { type: Number, default: 2 },
  colors: { type: Array, default: () => ['#008FFB', '#00E396', '#FEB019', '#FF4560', '#775DD0'] },
  showLegend: { type: Boolean, default: true },
  showDownload: { type: Boolean, default: true },
  donutLabels: { type: Boolean, default: true },
...
const baseOptions = computed(() => ({
  ...
  plotOptions: {
    pie: {
      donut: {
        labels: {
          show: props.donutLabels,
          total: { show: props.donutLabels },
        },
      },
    },
  },
...
const downloadChart = () => {
  chartRef.value?.chart?.dataURI().then(({ imgURI }) => {
    ...
    a.href = imgURI
    a.download = `${props.title || 'donut-chart'}.png`
    a.click()
  })
}
