...
const props = defineProps({
  // data: [[x, y, size]]
  series: { type: Array, default: () => [ { name: 'SAMPLE', data: [[10,20,30],[20,30,15],[30,10,40]] } ] },
...
const baseOptions = computed(() => ({
  chart: { type: 'bubble', zoom: { enabled: true }, fontFamily: isRTL.value ? 'Tajawal, sans-serif' : 'Inter, Roboto, sans-serif' },
  dataLabels: { enabled: false },
  colors: props.colors,
  legend: { show: props.showLegend, position: 'bottom', labels: { colors: isDark.value ? '#fff' : '#000' } },
  xaxis: { tickAmount: 12, type: 'numeric', labels: { style: { colors: isDark.value ? '#aaa' : '#666' } } },
  yaxis: { max: 70, labels: { style: { colors: isDark.value ? '#aaa' : '#666' } } },
...
const downloadChart = () => { chartRef.value?.chart?.dataURI().then(({ imgURI }) => { const a = document.createElement('a'); a.href = imgURI; a.download = `${props.title || 'bubble'}.png`; a.click() }) }
