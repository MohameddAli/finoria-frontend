...
const props = defineProps({
  series: { type: Array, default: () => [ { name: 'A', data: [[16.4, 5.4],[21.7, 2],[25.4, 3]] } ] },
...
const baseOptions = computed(() => ({
  chart: { type: 'scatter', zoom: { enabled: true, type: 'xy' }, fontFamily: isRTL.value ? 'Tajawal, sans-serif' : 'Inter, Roboto, sans-serif' },
  xaxis: { tickAmount: 10, labels: { style: { colors: isDark.value ? '#aaa' : '#666' } } },
  yaxis: { tickAmount: 7, labels: { style: { colors: isDark.value ? '#aaa' : '#666' } } },
  colors: props.colors,
...
const downloadChart = () => { chartRef.value?.chart?.dataURI().then(({ imgURI }) => { const a = document.createElement('a'); a.href = imgURI; a.download = `${props.title || 'scatter'}.png`; a.click() }) }
