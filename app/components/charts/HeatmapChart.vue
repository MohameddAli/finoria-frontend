...
const props = defineProps({
  series: { type: Array, default: () => [
    { name: 'Mon', data: [ { x: '1', y: 10 }, { x: '2', y: 20 }, { x: '3', y: 30 } ] },
    { name: 'Tue', data: [ { x: '1', y: 15 }, { x: '2', y: 25 }, { x: '3', y: 35 } ] }
  ] },
...
const baseOptions = computed(() => ({
  chart: { type: 'heatmap', fontFamily: isRTL.value ? 'Tajawal, sans-serif' : 'Inter, Roboto, sans-serif' },
...
const downloadChart = () => {
  chartRef.value?.chart?.dataURI().then(({ imgURI }) => { const a = document.createElement('a'); a.href = imgURI; a.download = `${props.title || 'heatmap'}.png`; a.click() })
}
